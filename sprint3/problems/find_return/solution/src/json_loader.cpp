...
model::Game LoadGame(const std::filesystem::path& json_path) {
	...
	if (value.as_object().contains("defaultBagCapacity")) {
		default_bag_capacity = static_cast<size_t>(value.as_object().at("defaultBagCapacity").as_int64());
	}
	...
	for (const auto& map_json : maps) {
		...
		if (map_json.as_object().contains("bagCapacity")) {
			bag_capacity = static_cast<size_t>(map_json.as_object().at("bagCapacity").as_int64());
		}
		...
		for (const auto& loot_type : loot_types) {
			...
			if (obj.contains("value")) {
				loot.value = static_cast<int>(obj.at("value").as_int64());
			}
