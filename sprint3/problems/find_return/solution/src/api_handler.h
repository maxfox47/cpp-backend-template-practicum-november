...
class ApiHandler {
 ...
 private:
	...
	void TickRequest(const http::request<Body, http::basic_fields<Allocator>>& req, Send&& send) {
		...
		const auto& time_delta_value = (*obj).at("timeDelta");
		double time_delta = 0.0;
		if (time_delta_value.is_int64()) {
			time_delta = static_cast<double>(time_delta_value.as_int64());
		} else if (time_delta_value.is_double()) {
			time_delta = time_delta_value.as_double();
		}
