...
// Для красивого вывода событий при провале теста
namespace Catch {
template <>
...
SCENARIO("FindGatherEvents detects single collision with correct data") {
	GIVEN("one moving gatherer and one item exactly on its path") {
		...
		// Собирающий движется по оси X из (0,0) в (10,0)
		provider.gatherers.push_back(
			 Gatherer{geom::Point2D{0.0, 0.0}, geom::Point2D{10.0, 0.0}, 0.0});
		// Предмет лежит посередине отрезка
		provider.items.push_back(Item{geom::Point2D{5.0, 0.0}, 0.0});
...
SCENARIO("FindGatherEvents does not report collisions when item is far from path") {
	GIVEN("one moving gatherer and a distant item") {
		...
		provider.gatherers.push_back(
			 Gatherer{geom::Point2D{0.0, 0.0}, geom::Point2D{10.0, 0.0}, 0.1});
		// Предмет далеко по оси Y, радиусов явно не хватает
		provider.items.push_back(Item{geom::Point2D{5.0, 10.0}, 0.1});
...
SCENARIO("FindGatherEvents returns all collisions in chronological order") {
	GIVEN("one gatherer passing through several items along its path") {
		...
		// Отрезок по X: [0, 4]
		provider.gatherers.push_back(Gatherer{geom::Point2D{0.0, 0.0}, geom::Point2D{4.0, 0.0}, 0.0});

		// Предмет до начала отрезка - столкновения быть не должно
		provider.items.push_back(Item{geom::Point2D{-1.0, 0.0}, 0.0}); // id = 0
		// Предмет в точке x = 1 (t = 0.25)
		provider.items.push_back(Item{geom::Point2D{1.0, 0.0}, 0.0}); // id = 1
		// Предмет в точке x = 3 (t = 0.75)
		provider.items.push_back(Item{geom::Point2D{3.0, 0.0}, 0.0}); // id = 2
		...
		WHEN("gather events are searched") {
			...
			THEN("only items on the segment are collected and events are ordered by time") {
				...
				// Проверим, что времена не убывают
				...
				// Первый собранный предмет - с x = 1
				...
				// Второй собранный предмет - с x = 3
...
SCENARIO("Stationary gatherer does not produce collision events") {
	GIVEN("a gatherer that did not move and an item at the same point") {
		...
		// Нулевое перемещение
		provider.gatherers.push_back(Gatherer{geom::Point2D{1.0, 1.0}, geom::Point2D{1.0, 1.0}, 1.0});
		provider.items.push_back(Item{geom::Point2D{1.0, 1.0}, 1.0});
