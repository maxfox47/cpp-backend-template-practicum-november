...
class Player {
 public:
	...
	double GetPlayTime() const { return dog_->GetPlayTime(); }

	double GetIdleTime() const { return dog_->GetIdleTime(); }
...
class PlayerTokens {
 public:
	...
	void RemoveToken(Token token) { token_to_player_.erase(token); }
