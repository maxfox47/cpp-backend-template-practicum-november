...
class ApiHandler {
 ...
 private:
	...
	// Парсит query-параметры start и maxItems, валидирует их и возвращает записи из БД
	template <typename Body, typename Allocator, typename Send>
	void RecordsRequest(const http::request<Body, http::basic_fields<Allocator>>& req, Send&& send) {
		auto ver = req.version();
		// Проверяем, что используется только метод GET
		CheckMethod(req, send, "GET");

		int start_index = 0;
		int max_items_count = 100;

		// Парсим query-параметры из URL (например, "?start=0&maxItems=100")
			...
			while (pos < query.size()) {
				...
				try {
					if (key == "start") {
						...
						// Проверяем, что maxItems не превышает максимально допустимое значение
	...
	// Формирует успешный ответ с записями об ушедших игроках
