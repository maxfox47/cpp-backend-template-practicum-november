...
ApiHandler::StringResponse ApiHandler::GoodRecordsRequest(const StringRequest& req, int start_index,
																			 int max_items_count) {
	try {
		// Получаем записи из базы данных с указанными параметрами пагинации
		auto records = database_.GetRecords(start_index, max_items_count);

		// Формируем JSON-массив из записей
		json::array records_array;
		for (const auto& record : records) {
			json::object record_obj;
			record_obj["name"] = record.name;
			record_obj["score"] = record.score;
			record_obj["playTime"] = record.play_time;
			records_array.push_back(record_obj);
		}

		std::string json_str = json::serialize(records_array);
