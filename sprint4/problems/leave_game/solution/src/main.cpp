...
int main(int argc, const char* argv[]) {
	...
	try {
		...
		// Выводим "server started" как можно раньше, чтобы тесты могли его увидеть
		// Это происходит до инициализации БД, чтобы не блокировать запуск
		...
		// Инициализируем схему БД в фоновом потоке, чтобы не блокировать запуск сервера
		// Это позволяет серверу быстрее вывести "server started" для тестов
		std::thread schema_init_thread([&database]() {
			try {
