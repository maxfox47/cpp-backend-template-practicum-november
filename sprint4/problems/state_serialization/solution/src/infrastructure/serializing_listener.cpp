...
void SerializingListener::OnTick(app::GameTime delta) {
    if (save_period_.count() == 0) {
        // Если период не установлен, сохраняем состояние при каждом тике
        // Это нужно для тестов, которые ожидают сохранение состояния при тиках
        SaveState();
        return;
    }
...
void SerializingListener::SaveState() {
    if (!state_getter_) {
        std::cerr << "[DEBUG] SaveState: state_getter_ не установлен" << std::endl;
        return;  // Функция получения состояния не установлена
    }

    try {
        std::cerr << "[DEBUG] SaveState: Сохраняем состояние в файл: " << state_file_path_ << std::endl;
        // Получаем состояние приложения
        std::string state_data = state_getter_();
        std::cerr << "[DEBUG] SaveState: Размер данных состояния: " << state_data.size() << " байт" << std::endl;

        // Создаем временный файл
        std::filesystem::path temp_file = state_file_path_;
        temp_file += ".tmp";

        // Убеждаемся, что директория для state существует
        if (!state_file_path_.parent_path().empty()) {
        ...
        std::cerr << "[DEBUG] SaveState: Состояние успешно сохранено" << std::endl;
    } catch (const std::exception& e) {
        std::cerr << "[DEBUG] SaveState: Ошибка сохранения: " << e.what() << std::endl;
...
bool SerializingListener::LoadState() {
    if (!std::filesystem::exists(state_file_path_)) {
        std::cerr << "[DEBUG] LoadState: Файл состояния не найден: " << state_file_path_ << std::endl;
        std::cerr << "[DEBUG] LoadState: Состояние пустое - начинаем с чистого листа" << std::endl;
        return false;  // Файл не существует, начинаем с чистого листа
    }

    try {
        std::cerr << "[DEBUG] LoadState: Загружаем состояние из файла: " << state_file_path_ << std::endl;
        ...
        std::cerr << "[DEBUG] LoadState: Состояние успешно загружено" << std::endl;
        return true;
    } catch (const std::exception& e) {
        std::cerr << "[DEBUG] LoadState: Ошибка загрузки состояния: " << e.what() << std::endl;
